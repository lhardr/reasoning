"""
Standalone candidate test: OpenRouter's Auto Router BETA (`openrouter/auto-beta`)
against our facit — NOT a panel addition.

`openrouter/auto` (the original slug, tested in run_auto_router.py /
results/auto/20260726T111445_auto.jsonl / docs/cc_debrief_auto_router.md) is
deprecated in favor of `openrouter/auto-beta`. This script re-runs the exact
same design against the new slug so the two are directly comparable, cell
for cell.

DEFAULT SETTINGS ONLY: no `plugins` parameter is sent (OpenRouter's Auto
Router beta accepts an optional cost_quality_tradeoff dial via a plugins
config; we deliberately never set it, to measure what an ordinary caller who
never touches that knob actually gets — which per OpenRouter's own docs
defaults to a dial value of 9 when unset. That default is a property of the
platform, not something this script requests). See docs/cc_debrief_auto_beta.md
for the full write-up including this caveat.

SESSION-STICKINESS: OpenRouter's routers can cache/pin a routing decision
within a short window (a few minutes) for requests that look like the same
session. This script never sends a `user` field, a session id, or any other
identity/continuity signal on any call — every one of the 80 calls is its
own, independent, anonymous request. Every row also logs
`session_id_sent: false` explicitly so this can be audited from the JSONL
alone. Whether passes 2-5 within a cell actually land on a different model
than pass 1 is then an empirical question, checked post-run (see the
debrief) — this script's only job is to not introduce a reason for them not
to.

Runs the exact same 16 task-cells as the panel (identical to
run_auto_router.py):
...
without this the run is worthless):
  ...
  - request_model_id: always "openrouter/auto-beta" — the literal string sent
  ...
  - session_id_sent (always false — see SESSION-STICKINESS above)

Writes to results/auto/<run_id>.jsonl — a new file, new run_id. Never
touches results/heavy, results/full, reasoning-data.json,
results/auto/20260726T111445_auto.jsonl, or docs/reasoning_findings.md.
Does not modify run_auto_router.py.
...
AUTO_SLUG = "openrouter/auto-beta"
MODEL_KEY = "openrouter_auto_beta"
...
# Verbatim from run.py's TOOLS3_INVITATION / HEAVY_INVITATION and from
# run_auto_router.py — same source, unchanged, so auto vs auto-beta is an
...
# ---------------------------------------------------------------------------
# Auto Router (beta) adapter — same OpenAI-compatible-dialect convention as
# run_auto_router.py's OpenRouterAutoAdapter, but pointed at the -beta slug.
# No panel.yaml entry, no dated pin, no plugins param, no session/user field.
# ---------------------------------------------------------------------------
...
class OpenRouterAutoBetaAdapter:
    def __init__(self) -> None:
        key = os.environ.get("OPENROUTER_API_KEY")
        if not key:
            raise RuntimeError("OPENROUTER_API_KEY not set — required for openrouter/auto-beta")
        self.client = OpenAI(api_key=key, base_url=OPENROUTER_BASE_URL)

    def _base_extra_body(self) -> dict:
        # Deliberately NO "plugins" key — default settings only, no
        # cost_quality_tradeoff override. Deliberately no "user" field — no
        # session/identity continuity signal, see module docstring.
        return {
    ...
    def call(self, prompt: str) -> ModelResponse:
        try:
            ...
            raise AdapterError(f"openrouter_auto_beta API error: {exc}") from exc
...
def run() -> None:
    run_id = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S") + "_autobeta"
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)

    adapter = OpenRouterAutoBetaAdapter()
    ...
    print(f"model={AUTO_SLUG}  plugins_param_sent=False  cost_quality_tradeoff=platform_default(9)  session_id_sent=False")
    print("-" * 100)
        ...
        for pass_index in range(1, N_PASSES + 1):
            ...
            try:
                ...
                extra.update({
                    ...
                    "plugins_param_sent": False,
                    "cost_quality_tradeoff": "platform_default(9)_not_overridden",
    ...
    meta = {
        "run_id": run_id,
        "purpose": (
            "Standalone candidate test of openrouter/auto-beta against panel facit "
            "— not a panel member. Successor slug to openrouter/auto (deprecated); "
            "see results/auto/20260726T111445_auto.jsonl / "
            "docs/cc_debrief_auto_router.md for the predecessor run this is compared "
            "against in docs/cc_debrief_auto_beta.md."
        ),
        "request_model_id": AUTO_SLUG,
        "reasoning_effort": REASONING_EFFORT,
        "thinking_budget": THINKING_BUDGET,
